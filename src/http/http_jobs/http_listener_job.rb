module ROM
	class HTTPListenerJob < ROM::Job
		...
		def initialize(tcp_server, job_pool, https = false, cert = nil, redirect = nil)
		...
		def job_task(log)
			loop do
				...
				begin
					respond_job = HTTPRespondJob.new(con, @redirect)
					@job_pool.add_job(respond_job)
				rescue Exception => ex
					log.error('Failed create an HTTP response job!', ex)
				end
			end
		end
